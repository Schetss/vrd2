...
class Category extends FrontendBaseBlock
{
	...
	private function getData()
	{
		...
		// get by URL
		$this->record = FrontendJobsModel::getCategory($this->URL->getParameter(1));
		
		// anything found?
		if(empty($this->record)) $this->redirect(FrontendNavigation::getURL(404));
		
		// overwrite URLs
		$this->record['full_url'] = FrontendNavigation::getURLForBlock('Jobs', 'Category') . '/' . $this->record['url'];
		$this->jobs = FrontendJobsModel::getAllForCategory($this->record['id']);
	}
