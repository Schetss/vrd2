...
namespace Frontend\Modules\Contact\Engine;
...
class Model
{
    ...
    public static function getAll()
    {
        ...
        $queryCategories = 'SELECT c.id, c.title
                    ...
                    RIGHT OUTER JOIN contact_albums as a ON c.id = a.category_id
                    ORDER BY c.title';
        $categoriesWithAlbums = $db->getRecords($queryCategories);

        // get albums
        $queryAlbums = 'SELECT a.*, m.url
                    FROM contact_albums AS a
        ...
        // init var
        $link = FrontendNavigation::getURLForBlock('Contact', 'detail');
            ...
            foreach($categoriesWithAlbums as $category) {
                ...
                foreach($albums as $key => $album) {
                    if($album['category_id'] == $category['id']) {
                        $contact[$category['id']]['albums'][$key] = $album;
                        $contact[$category['id']]['albums'][$key]['full_url'] = $link . '/' . $album['url'];
                    }
                }
            }
        }
    ...
    public static function getById($id)
    {
        ...
        $tracks = (array) FrontendModel::getContainer()->get('database')->getRecords(
            ...
            t.sequence
            FROM contact_albums AS a
            INNER JOIN contact_albums_tracks AS t ON t.album_id = a.id
            WHERE a.id = ? AND a.hidden = ?
            ORDER BY t.sequence',
            array((int) $id, 'N')
        );
        ...
        $albumDetailUrl = FrontendNavigation::getURLForBlock('Contact', 'detail');
