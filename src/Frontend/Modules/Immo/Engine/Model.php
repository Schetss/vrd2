...
namespace Frontend\Modules\Immo\Engine;
...
class Model implements FrontendTagsInterface
{
    ...
    public static function get($url)
    {
        $item = (array)FrontendModel::getContainer()->get('database')->getRecord(
            'SELECT i.*, UNIX_TIMESTAMP(i.created_on) AS created_on,
             ...
             INNER JOIN immo_clients AS cl ON i.client_id = cl.id
             INNER JOIN immo_categories AS c ON i.category_id = c.id
    ...
    public static function getAll($limit = 10, $offset = 0)
    {
        ...
        $detailUrl = FrontendNavigation::getURLForBlock('Immo', 'Detail');
        $categoryUrl = FrontendNavigation::getURLForBlock('Immo', 'Category');
        $clientUrl = FrontendNavigation::getURLForBlock('Immo', 'Client');

        // prepare items
        $numberOfImagesToShowInList = FrontendModel::getModuleSetting('Immo', 'overview_num_of_images', 1);
     ...
     * @return array
     */
    public static function getImmoByIds(array $Ids)
    {
        if (empty($Ids)) {
            return array();
        }
    ...
    public static function getAllForCategory($categoryId, $limit = null, $excludeIds = null)
    {
        ...
        // init var
        $link = FrontendNavigation::getURLForBlock('Immo', 'Detail');
    ...
    public static function getCategories()
    {
        ...
        $link = FrontendNavigation::getURLForBlock('Immo', 'Category');
    ...
    public static function getClients()
    {
        ...
        $link = FrontendNavigation::getURLForBlock('Immo', 'Client');

        // build the item urls and image
        foreach ($items as &$item) {
            // thumb
            if($item['image'] != null){
                $item['image'] = FRONTEND_FILES_URL . '/Immo/references/300x200/' . $item['image']; 
            }
    ...
    {
        $settings = FrontendModel::getModuleSettings('Immo');
        ...
        foreach ($items as &$item) {
            foreach ($imageFormats as $format_name => $format) {
                $item['sizes'][$format_name] = FRONTEND_FILES_URL . '/Immo/' . $item['immo_id'] . '/' . $format . '/' . $item['filename'];
            }
        }
    ...
    public static function getRelatedImmo($id)
    {
        ...
        foreach ($relatedIds as $relatedImmo) {
            $relatedImmo[] = self::get(null, $relatedImmo['related_immo_id']);
        }
     ...
     * @return array
     */
    public static function getSpotlightImmo($url = null)
    {
        if ($url) {
            $item = (array)FrontendModel::getContainer()->get('database')->getRecord(
        ...
        if ($item) {
            $img = FrontendModel::getContainer()->get('database')->getRecord('SELECT filename FROM immo_images WHERE immo_id = ? ORDER BY sequence LIMIT 1', array((int)$item['id']));
            if ($img) {
                $item['image'] = FRONTEND_FILES_URL . '/Immo/' . $item['id'] . '/295x195/' . $img['filename'];
                $item['image_header'] = FRONTEND_FILES_URL . '/Immo/' . $item['id'] . '/1920x456/' . $img['filename'];
            };
            $item['full_url'] = FrontendNavigation::getURLForBlock('Immo', 'Detail') . '/' . $item['url'];
        }
    ...
    public static function search(array $ids)
    {
        $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.id, i.title, i.text AS text, m.url,
             ...
             FROM immo AS i
             INNER JOIN meta AS m ON i.meta_id = m.id
             INNER JOIN immo_categories AS c ON c.id = i.category_id
             INNER JOIN meta AS m2 ON c.meta_id = m2.id
