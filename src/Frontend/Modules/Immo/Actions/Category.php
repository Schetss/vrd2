...
class Category extends FrontendBaseBlock
{
	...
	private function getData()
	{
		...
		// get by URL
		$this->record = FrontendImmoModel::getCategory($this->URL->getParameter(1));
		
		// anything found?
		if(empty($this->record)) $this->redirect(FrontendNavigation::getURL(404));
		
		// overwrite URLs
		$this->record['full_url'] = FrontendNavigation::getURLForBlock('Immo', 'Category') . '/' . $this->record['url'];
		$this->immo = FrontendImmoModel::getAllForCategory($this->record['id']);
	}
