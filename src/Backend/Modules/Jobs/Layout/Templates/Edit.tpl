...
<div class="pageTitle">
	<h2>{$lblJobs|ucfirst}: {$msgEditJobs|sprintf:{$item.title}}</h2>
</div>

{form:edit}

	<label for="title">{$lblJobs|ucfirst}</label>
			...
			<table width="100%">
				<tr>
					...
					<td id="sidebar">
						...
						<div id="publishOptions" class="box">
							<div class="heading">
								<h3>{$lblRelatedJobs|ucfirst}</h3>
							</div>
							<div class="options">
								<label for="spotlight">{$msgRelatedJobs}</label></br>
								{$ddmRelatedJobs} {$ddmRelatedJobsError}
							</div>
						</div>
					</td>
				</tr>
