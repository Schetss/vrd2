/**
 * Interaction for the jobs categories
 *
...
jsBackend.jobs =
{
	...
	{
		// index stuff
		if($('#dataGridJobsHolder').length > 0)
		{
			// destroy default drag and drop
			$('.sequenceByDragAndDrop tbody').sortable('destroy');

			// drag and drop
			jsBackend.jobs.bindDragAndDropCategoryJobs();
			jsBackend.jobs.checkForEmptyCategories();
		}
	...
	/**
	 * Check for empty categories and make it still possible to drop jobs
	 */
	checkForEmptyCategories: function()
	{
		...
		$('table.emptyGrid').each(function(){
			...
			$(this).append('<tr class="noJobs"><td colspan="' + $(this).find('th').length + '">' + jsBackend.locale.msg('NoJobsInCategory') +'</td></tr>');
			$(this).removeClass('emptyGrid');
		});

		// when there are empty categories
		if($('tr.noJobs').length > 0)
		{
			...
			$('table.dataGrid').droppable(
			{
				...
				drop: function(e, ui)
				{
					// remove the no jobs in category message
					$(this).find('tr.noJobs').remove();
				}
			});

			// cleanup remaining no jobs
			$('table.dataGrid').each(function(){
				if($(this).find('tr').length > 2) $(this).find('tr.noJobs').remove();
			});
		}
	},
	 ...
	 */
	bindDragAndDropCategoryJobs: function()
	{
		// jobs_images_dg 
		$.each($('#jobs_images_dg'), function()
		{
			// make them sortable
						...
						$.ajax(
								{
									...
									success: function(data, textStatus)
									{
										...
										if(data.code == 200)
										{
											...
											{
												$('div#dataGrid-' + fromCategoryId + ' table.dataGrid').append('<tr class="noJobs"><td colspan="4">' + jsBackend.locale.msg('NoJobsInCategory') + '</td></tr>');
											}
