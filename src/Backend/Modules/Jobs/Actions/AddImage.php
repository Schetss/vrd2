...
use Backend\Modules\Jobs\Engine\Helper as BackendJobsHelper;
 
/**
 * This is the add action, it will display a form to add an image to a job.
 *
	...
	private function validateForm()
	{
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$path = FRONTEND_FILES_PATH . '/' . $this->module . '/' . $item['job_id'];
				...
				$item['sequence'] = BackendJobsModel::getMaximumImagesSequence($item['job_id'])+1;

				// add images
				BackendJobsHelper::addImages($image, $path, $item['filename'], $formats);

				// save the item
				$item['id'] = BackendJobsModel::saveImage($item);
