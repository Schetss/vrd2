...
use Backend\Modules\Jobs\Engine\Model as BackendJobsHelper;
use Backend\Modules\Jobs\Engine\Model as BackendJobsModel;
 
/**
 * This is the edit image action, it will display a form to edit an existing job image.
 *
...
class EditImage extends BackendBaseActionEdit
{
	...
	public function execute()
	{
		...
		if($this->id !== null && BackendJobsModel::existsImage($this->id))
		{
			parent::execute();
	...
	protected function getData()
	{
		...
		$this->image = BackendJobsModel::getImage($this->getParameter('id', 'int'));
	...
	protected function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$path = FRONTEND_FILES_PATH . '/' . $this->module . '/' . $this->job['id'];
				$formats = array();
				...
				if($image->isFilled())
				{
					...
					// add images
					BackendJobsHelper::addImages($image, $path, $item['filename'], $formats);
				}
				
				// save the item
				$id = BackendJobsModel::saveImage($item);
