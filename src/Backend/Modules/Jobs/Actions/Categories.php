...
/**
 * This is the categories action, it will display the overview of job categories.
 *
...
class Categories extends BackendBaseActionIndex
{
	...
	private function loadDataGrid()
	{
		...
		$this->multipleCategoriesAllowed = BackendModel::getModuleSetting('jobs', 'allow_multiple_categories', true);

		// create dataGrid
		$this->dataGrid = new BackendDataGridDB(BackendJobsModel::QRY_DATAGRID_BROWSE_CATEGORIES, BL::getWorkingLanguage());
	...
	protected function parse()
	{
		...
		{
			$this->tpl->assign('showJobsAddCategory', true);
		}
		else $this->tpl->assign('showJobsAddCategory', false);
	}
	...
	public static function setClickableCount($count, $link)
	{
		...
		if($count > 1) return '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Jobs') . '</a>';
		if($count == 1) return '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Jobs') . '</a>';
		else return BL::getLabel('NoJobs');
	}
}
