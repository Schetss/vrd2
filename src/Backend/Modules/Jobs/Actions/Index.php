...
/**
 * This is the index action (default), it will display the overview of jobs.
 *
...
class Index extends BackendBaseActionIndex
{
	...
	{
		// load all categories
		$categories = BackendJobsModel::getCategories(true);
		
		// loop categories and create a dataGrid for each one
		foreach($categories as $categoryId => $categoryTitle)
		{
			$dataGrid = new BackendDataGridDB(BackendJobsModel::QRY_DATAGRID_BROWSE, array(BL::getWorkingLanguage(), $categoryId));
			
			$dataGrid->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
			$dataGrid->setAttributes(array('id' => 'jobs_dg'));
		...
		// set empty datagrid
		$this->emptyDatagrid = new BackendDataGridArray(array(array('dragAndDropHandle' => '', 'title' => BL::msg('NoJobsInCategory'), 'edit' => '')));
	 ...
	 *
	 * @param int $jobId The specific id of the job
	 * @return string
	 */
	public static function setMediaLink($jobId)
	{
		return '<a class="button icon iconEdit linkButton" href="' . BackendModel::createURLForAction('media') . '&job_id=' . $jobId . '">
	...
	public static function setClientLink($clientId, $client)
	{
		...
		} else return '<span>' . BL::lbl('NoJobsReference') . '</span>';	
	}
}
