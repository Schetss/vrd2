...
class EditClient extends BackendBaseActionEdit
{
	...
	{
		$this->record = BackendJobsModel::getClient($this->id);
	}
	...
	protected function parse()
	{
		...
		$this->tpl->assign('showJobsDeleteClient', BackendJobsModel::deleteClientAllowed($this->id) && BackendAuthentication::isAllowedAction('delete_client'));
	}
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setUrlCallback('Backend\Modules\Jobs\Engine\Model', 'getURLForClient', array($this->record['id']));
			...
			if($this->frm->isCorrect())
			{
				...
				// update the item
				BackendJobsModel::updateClient($item);
