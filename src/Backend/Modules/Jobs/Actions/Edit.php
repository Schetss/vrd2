...
/**
 * This is the edit action, it will display a form to edit an existing job.
 *
...
class Edit extends BackendBaseActionEdit
{
	...
	{
		$this->record = (array) BackendJobsModel::get($this->id);
		$this->categories = (array) BackendJobsModel::getCategories();
		$this->clients = (array) BackendJobsModel::getClients();
		$this->allJobsGroupedByCategories = (array) BackendJobsModel::getAllJobsGroupedByCategories();
		$this->relatedJobs = (array) BackendJobsModel::getRelatedJobs($this->id);
	}
	...
	private function loadForm()
	{
		...
		$this->frm->addDropdown('related_jobs', $this->allJobsGroupedByCategories, $this->relatedJobs, true );
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setUrlCallback('Backend\Modules\Jobs\Engine\Model', 'getURL', array($this->record['id']));
			...
			if($this->frm->isCorrect())
			{
				...
				BackendJobsModel::saveRelatedJobs($item['id'], $this->frm->getField('related_jobs')->getValue(), $this->relatedJobs);
				
				BackendModel::triggerEvent($this->getModule(), 'after_edit', array('item' => $item));

				// edit search index
				BackendSearchModel::saveIndex('jobs', $item['id'], array('title' => $item['title'], 'text' => $item['text']));
