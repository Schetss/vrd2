...
/**
 * This is the media action, it will display the overview of media for a specific job.
 *
...
class Media extends BackendBaseActionIndex
{
	...
	{
		$this->job = BackendJobsModel::get($this->id);
	}
	...
	{
		// set image link
		$imageLink = FRONTEND_FILES_URL . '/' . $this->module . '/[job_id]/64x64';
	  
		// create images datagrid
		$this->dgImages = new BackendDataGridDB(BackendJobsModel::QRY_DATAGRID_BROWSE_IMAGES, $this->id);
		$this->dgImages->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
		$this->dgImages->setAttributes(array('id' => 'jobs_images_dg'));
		$this->dgImages->setAttributes(array('data-action' => 'sequence_images'));			
		  
		$this->dgImages->setColumnHidden('sequence');
		$this->dgImages->setColumnHidden('job_id');
		...
		$this->dgImages->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_image') . '&amp;id=[id]&amp;job_id=[job_id]', BL::lbl('Edit'));
	...
	protected function loadDataGridFiles()
	{
		...
		$this->dgFiles = new BackendDataGridDB(BackendJobsModel::QRY_DATAGRID_BROWSE_FILES, $this->id);
		
		$this->dgFiles->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
		$this->dgFiles->setAttributes(array('id' => 'jobs_files_dg'));
		$this->dgFiles->setAttributes(array('data-action' => 'sequence_files'));
	      
		$this->dgFiles->setColumnHidden('sequence');
		$this->dgFiles->setColumnHidden('job_id');
		...
		$this->dgFiles->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_file') . '&amp;id=[id]&amp;job_id=[job_id]', BL::lbl('Edit'));      
	...
	protected function loadDataGridVideos()
	{
		...
		$this->dgVideos = new BackendDataGridDB(BackendJobsModel::QRY_DATAGRID_BROWSE_VIDEOS, $this->id);
		  
		$this->dgVideos->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
		$this->dgVideos->setAttributes(array('id' => 'jobs_videos_dg'));
		$this->dgVideos->setAttributes(array('data-action' => 'sequence_videos'));
	      
		$this->dgVideos->setColumnHidden('job_id');
		...
		$this->dgVideos->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_video') . '&amp;id=[id]&amp;job_id=[job_id]', BL::lbl('Edit'));
