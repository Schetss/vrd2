...
/**
 * This is the add action, it will display a form to create a new job.
 *
...
class Add extends BackendBaseActionAdd
{
	...
	private function loadForm()
	{
		...
		$categories = BackendJobsModel::getCategories();
		$clients = BackendJobsModel::getClients();
		$allJobsGroupedByCategories = BackendJobsModel::getAllJobsGroupedByCategories();
		...
		$this->frm->addDropdown('related_jobs', $allJobsGroupedByCategories, null, true);
	...
	private function validateForm()
	{
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = BackendJobsModel::getMaximumSequence($this->frm->getField('category_id')->getValue()) + 1;
				
				// save the data
				$item['id'] = BackendJobsModel::insert($item);
				BackendTagsModel::saveTags($item['id'], $this->frm->getField('tags')->getValue(), $this->URL->getModule());
				BackendJobsModel::saveRelatedJobs($item['id'], $this->frm->getField('related_jobs')->getValue());
