...
class AddCategory extends BackendBaseActionAdd
{
	...
	public function execute()
	{
		...
		if(!BackendModel::getModuleSetting('jobs', 'allow_multiple_categories', true)) $this->redirect(BackendModel::createURLForAction('categories') . '&error=only-one-category-allowed');
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setURLCallback('Backend\Modules\Jobs\Engine\Model', 'getURLForCategory');
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = BackendJobsModel::getMaximumCategorySequence() + 1;

				// save the data
				$item['id'] = BackendJobsModel::insertCategory($item);
