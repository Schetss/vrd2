...
class AddClient extends BackendBaseActionAdd
{
	...
	public function execute()
	{
		...
		if(!BackendModel::getModuleSetting('jobs', 'allow_multiple_clients', true)) $this->redirect(BackendModel::createURLForAction('clients') . '&error=only-one-client-allowed');
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setURLCallback('Backend\Modules\Jobs\Engine\Model', 'getURLForClient');
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = BackendJobsModel::getMaximumClientSequence() + 1;
				...
				// save the data
				$item['id'] = BackendJobsModel::insertClient($item);
