...
namespace Backend\Modules\Contact\Engine;
...
/**
 * In this file we store all generic functions that we will be using in the contact module
 *
...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE =
        ...
        FROM contact_albums AS e
        LEFT OUTER JOIN contact_categories as c ON e.category_id = c.id';

    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(a.id) AS num_items
        FROM contact_categories AS c
        LEFT OUTER JOIN contact_albums AS a ON c.id = a.category_id
        GROUP BY c.id';
    ...
    {
        BackendModel::getContainer()->get('database')->delete('contact_albums', 'id = ?', (int) $id);
    }
    ...
    public static function existsTrack($id)
    {
        return (bool) BackendModel::getContainer()->get('database')->getVar(
            'SELECT 1
             FROM contact_albums_tracks AS i
             WHERE i.id = ?
             LIMIT 1',
            array((int) $id)
        );
        ...
        if($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (", COUNT(p.category_id) ,")") AS title
                 FROM contact_categories AS i
                 LEFT OUTER JOIN contact_albums AS p ON i.id = p.category_id AND p.status = ?
                 WHERE i.language = ?
                 GROUP BY i.id',
                array('active')
            );
    ...
    public static function getTracks($id)
    {
        return (array) BackendModel::getContainer()->get('database')->getRecords(
            ...
            FROM contact_albums_tracks AS t
            WHERE t.album_id = ?
            ORDER BY t.sequence',
            array($id)
        );
    ...
    public static function insert(array $data)
    {
        ...
        return (int) BackendModel::getContainer()->get('database')->insert('contact_albums', $data);
    }
    ...
    public static function insertTrack(array $data)
    {
        ...
        return (int) BackendModel::getContainer()->get('database')->insert('contact_albums_tracks', $data);
    }
    ...
    public static function insertCategory(array $item, $meta = null)
    {
        ...
        // create category
        $item['id'] = $db->insert('contact_categories', $item);
    ...
    public static function deleteCategory($id)
    {
        ...
        if(!empty($item)) {
            ...
            // delete category
            $db->delete('contact_categories', 'id = ?', array($id));

            // update category for the posts that might be in this category
            $db->update('contact_albums', array('category_id' => null), 'category_id = ?', array($id));

            // invalidate the cache for contact
            BackendModel::invalidateFrontendCache('contact', BL::getWorkingLanguage());
        }
    }
    ...
    public static function updateCategory(array $item)
    {
        ...
        // update category
        $updated = $db->update('contact_categories', $item, 'id = ?', array((int) $item['id']));
    ...
    public static function updateTrack(array $item)
    {
        ...
        // update track
        $updated = $db->update('contact_albums_tracks', $item, 'id = ?', array((int) $item['id']));
    ...
    {
        BackendModel::getContainer()->get('database')->delete('contact_albums_tracks', 'id = ?', (int) $id);
    }
