...
class AddClient extends BackendBaseActionAdd
{
	...
	public function execute()
	{
		...
		if(!BackendModel::getModuleSetting('immo', 'allow_multiple_clients', true)) $this->redirect(BackendModel::createURLForAction('clients') . '&error=only-one-client-allowed');
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setURLCallback('Backend\Modules\Immo\Engine\Model', 'getURLForClient');
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = BackendImmoModel::getMaximumClientSequence() + 1;
				...
				// save the data
				$item['id'] = BackendImmoModel::insertClient($item);
