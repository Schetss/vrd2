...
use Backend\Modules\Immo\Engine\Model as BackendImmoHelper;
use Backend\Modules\Immo\Engine\Model as BackendImmoModel;
 
/**
 * This is the edit image action, it will display a form to edit an existing immo image.
 *
...
class EditImage extends BackendBaseActionEdit
{
	...
	public function execute()
	{
		...
		if($this->id !== null && BackendImmoModel::existsImage($this->id))
		{
			parent::execute();
	...
	protected function getData()
	{
		...
		$this->image = BackendImmoModel::getImage($this->getParameter('id', 'int'));
	...
	protected function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$path = FRONTEND_FILES_PATH . '/' . $this->module . '/' . $this->immo['id'];
				$formats = array();
				...
				// save the item
				$id = BackendImmoModel::saveImage($item);

				// trigger event
				BackendModel::triggerEvent($this->getModule(), 'after_edit_image', array('item' => $item));

				// everything is saved, so redirect to the overview
				$this->redirect(BackendModel::createURLForAction('media') . '&immo_id=' . $this->immo['id'] . '&report=edited&var=' . urlencode($item['title']) . '&highlight=row-' . $item['id']);
			}
		}
	}
}
