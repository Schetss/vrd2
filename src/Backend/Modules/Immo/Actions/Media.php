...
/**
 * This is the media action, it will display the overview of media for a specific immo.
 *
...
class Media extends BackendBaseActionIndex
{
	...
	{
		$this->immo = BackendImmoModel::get($this->id);
	}
	...
	{
		// set image link
		$imageLink = FRONTEND_FILES_URL . '/' . $this->module . '/[immo_id]/64x64';
	  
		// create images datagrid
		$this->dgImages = new BackendDataGridDB(BackendImmoModel::QRY_DATAGRID_BROWSE_IMAGES, $this->id);
		$this->dgImages->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
		$this->dgImages->setAttributes(array('id' => 'immo_images_dg'));
		$this->dgImages->setAttributes(array('data-action' => 'sequence_images'));			
		  
		$this->dgImages->setColumnHidden('sequence');
		$this->dgImages->setColumnHidden('immo_id');
		...
		$this->dgImages->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_image') . '&amp;id=[id]&amp;immo_id=[immo_id]', BL::lbl('Edit'));
	...
	protected function loadDataGridFiles()
	{
		...
		$this->dgFiles = new BackendDataGridDB(BackendImmoModel::QRY_DATAGRID_BROWSE_FILES, $this->id);
		
		$this->dgFiles->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
		$this->dgFiles->setAttributes(array('id' => 'immo_files_dg'));
		$this->dgFiles->setAttributes(array('data-action' => 'sequence_files'));
	      
		$this->dgFiles->setColumnHidden('sequence');
		$this->dgFiles->setColumnHidden('immo_id');
		...
		$this->dgFiles->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_file') . '&amp;id=[id]&amp;immo_id=[immo_id]', BL::lbl('Edit'));      
	...
	protected function loadDataGridVideos()
	{
		...
		$this->dgVideos = new BackendDataGridDB(BackendImmoModel::QRY_DATAGRID_BROWSE_VIDEOS, $this->id);
		  
		$this->dgVideos->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
		$this->dgVideos->setAttributes(array('id' => 'immo_videos_dg'));
		$this->dgVideos->setAttributes(array('data-action' => 'sequence_videos'));
	      
		$this->dgVideos->setColumnHidden('immo_id');
		...
		$this->dgVideos->addColumn('edit', null, BL::lbl('Edit'), BackendModel::createURLForAction('edit_video') . '&amp;id=[id]&amp;immo_id=[immo_id]', BL::lbl('Edit'));
