...
/**
 * This is the edit action, it will display a form to edit an existing immo.
 *
...
class Edit extends BackendBaseActionEdit
{
	...
	{
		$this->record = (array) BackendImmoModel::get($this->id);
		$this->categories = (array) BackendImmoModel::getCategories();
		$this->clients = (array) BackendImmoModel::getClients();
		$this->allImmoGroupedByCategories = (array) BackendImmoModel::getAllImmoGroupedByCategories();
		$this->relatedImmo = (array) BackendImmoModel::getRelatedImmo($this->id);
	}
	...
	private function loadForm()
	{
		...
		$this->frm->addDropdown('related_immo', $this->allImmoGroupedByCategories, $this->relatedImmo, true );
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setUrlCallback('Backend\Modules\Immo\Engine\Model', 'getURL', array($this->record['id']));
			...
			if($this->frm->isCorrect())
			{
				...
				BackendImmoModel::saveRelatedImmo($item['id'], $this->frm->getField('related_immo')->getValue(), $this->relatedImmo);
				
				BackendModel::triggerEvent($this->getModule(), 'after_edit', array('item' => $item));

				// edit search index
				BackendSearchModel::saveIndex('immo', $item['id'], array('title' => $item['title'], 'text' => $item['text']));
