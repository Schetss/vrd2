...
/**
 * This is the index action (default), it will display the overview of immo.
 *
...
class Index extends BackendBaseActionIndex
{
	...
	{
		// load all categories
		$categories = BackendImmoModel::getCategories(true);
		
		// loop categories and create a dataGrid for each one
		foreach($categories as $categoryId => $categoryTitle)
		{
			$dataGrid = new BackendDataGridDB(BackendImmoModel::QRY_DATAGRID_BROWSE, array(BL::getWorkingLanguage(), $categoryId));
			
			$dataGrid->setAttributes(array('class' => 'dataGrid sequenceByDragAndDrop'));
			$dataGrid->setAttributes(array('id' => 'immo_dg'));
		...
		// set empty datagrid
		$this->emptyDatagrid = new BackendDataGridArray(array(array('dragAndDropHandle' => '', 'title' => BL::msg('NoImmoInCategory'), 'edit' => '')));
	 ...
	 *
	 * @param int $immoId The specific id of the immo
	 * @return string
	 */
	public static function setMediaLink($immoId)
	{
		return '<a class="button icon iconEdit linkButton" href="' . BackendModel::createURLForAction('media') . '&immo_id=' . $immoId . '">
	...
	public static function setClientLink($clientId, $client)
	{
		...
		} else return '<span>' . BL::lbl('NoImmoReference') . '</span>';	
	}
}
