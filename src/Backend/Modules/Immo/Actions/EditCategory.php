...
class EditCategory extends BackendBaseActionEdit
{
	...
	public function execute()
	{
		...
		// does the item exist?
		if($this->id !== null && BackendImmoModel::existsCategory($this->id))
		{
			parent::execute();
	...
	{
		$this->record = BackendImmoModel::getCategory($this->id);
	}
	...
	protected function parse()
	{
		...
		$this->tpl->assign('showImmoDeleteCategory', BackendImmoModel::deleteCategoryAllowed($this->id) && BackendAuthentication::isAllowedAction('delete_category'));
	}
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setUrlCallback('Backend\Modules\Immo\Engine\Model', 'getURLForCategory', array($this->record['id']));
			...
			if($this->frm->isCorrect())
			{
				...
				// update the item
				BackendImmoModel::updateCategory($item);
