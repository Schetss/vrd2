...
class EditClient extends BackendBaseActionEdit
{
	...
	{
		$this->record = BackendImmoModel::getClient($this->id);
	}
	...
	protected function parse()
	{
		...
		$this->tpl->assign('showImmoDeleteClient', BackendImmoModel::deleteClientAllowed($this->id) && BackendAuthentication::isAllowedAction('delete_client'));
	}
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setUrlCallback('Backend\Modules\Immo\Engine\Model', 'getURLForClient', array($this->record['id']));
			...
			if($this->frm->isCorrect())
			{
				...
				// update the item
				BackendImmoModel::updateClient($item);
