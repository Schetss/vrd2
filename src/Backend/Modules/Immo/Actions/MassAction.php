...
/**
 * This action is used to perform mass actions on immo media (delete, ...)
 *
...
class MassAction extends BackendBaseAction
{
	...
	public function execute()
	{
		...
		else
		{
			// redefine id's
			$aIds = (array) $_GET['id'];
			$slideshowID = (int) $_GET['immo_id'];

			// delete media
			if($action == 'deleteImages'){
				BackendImmoModel::deleteImage($aIds);
			} else if($action == 'deleteFiles'){
				BackendImmoModel::deleteFile($aIds);
			} else if($action == 'deleteVideos'){
				BackendImmoModel::deleteVideo($aIds);
			}
		}

		$this->redirect(BackendModel::createURLForAction('media') . '&immo_id=' . $slideshowID . '&report=deleted');
	}
}
