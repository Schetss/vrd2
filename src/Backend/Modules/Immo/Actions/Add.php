...
/**
 * This is the add action, it will display a form to create a new immo.
 *
...
class Add extends BackendBaseActionAdd
{
	...
	private function loadForm()
	{
		...
		$categories = BackendImmoModel::getCategories();
		$clients = BackendImmoModel::getClients();
		$allImmoGroupedByCategories = BackendImmoModel::getAllImmoGroupedByCategories();
		...
		$this->frm->addDropdown('related_immo', $allImmoGroupedByCategories, null, true);
	...
	private function validateForm()
	{
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = BackendImmoModel::getMaximumSequence($this->frm->getField('category_id')->getValue()) + 1;
				
				// save the data
				$item['id'] = BackendImmoModel::insert($item);
				BackendTagsModel::saveTags($item['id'], $this->frm->getField('tags')->getValue(), $this->URL->getModule());
				BackendImmoModel::saveRelatedImmo($item['id'], $this->frm->getField('related_immo')->getValue());
