...
class AddCategory extends BackendBaseActionAdd
{
	...
	public function execute()
	{
		...
		if(!BackendModel::getModuleSetting('immo', 'allow_multiple_categories', true)) $this->redirect(BackendModel::createURLForAction('categories') . '&error=only-one-category-allowed');
	...
	private function validateForm()
	{
		...
		{
			$this->meta->setURLCallback('Backend\Modules\Immo\Engine\Model', 'getURLForCategory');
			...
			if($this->frm->isCorrect())
			{
				...
				$item['sequence'] = BackendImmoModel::getMaximumCategorySequence() + 1;

				// save the data
				$item['id'] = BackendImmoModel::insertCategory($item);
