/**
 * Interaction for the immo categories
 *
...
jsBackend.immo =
{
	...
	{
		// index stuff
		if($('#dataGridImmoHolder').length > 0)
		{
			// destroy default drag and drop
			$('.sequenceByDragAndDrop tbody').sortable('destroy');

			// drag and drop
			jsBackend.immo.bindDragAndDropCategoryImmo();
			jsBackend.immo.checkForEmptyCategories();
		}
	...
	/**
	 * Check for empty categories and make it still possible to drop immo
	 */
	checkForEmptyCategories: function()
	{
		...
		$('table.emptyGrid').each(function(){
			...
			$(this).append('<tr class="noImmo"><td colspan="' + $(this).find('th').length + '">' + jsBackend.locale.msg('NoImmoInCategory') +'</td></tr>');
			$(this).removeClass('emptyGrid');
		});

		// when there are empty categories
		if($('tr.noImmo').length > 0)
		{
			...
			$('table.dataGrid').droppable(
			{
				...
				drop: function(e, ui)
				{
					// remove the no immo in category message
					$(this).find('tr.noImmo').remove();
				}
			});

			// cleanup remaining no immo
			$('table.dataGrid').each(function(){
				if($(this).find('tr').length > 2) $(this).find('tr.noImmo').remove();
			});
		}
	},
	 ...
	 */
	bindDragAndDropCategoryImmo: function()
	{
		// immo_images_dg 
		$.each($('#immo_images_dg'), function()
		{
			// make them sortable
						...
						$.ajax(
								{
									...
									success: function(data, textStatus)
									{
										...
										if(data.code == 200)
										{
											...
											{
												$('div#dataGrid-' + fromCategoryId + ' table.dataGrid').append('<tr class="noImmo"><td colspan="4">' + jsBackend.locale.msg('NoImmoInCategory') + '</td></tr>');
											}
